...
try {
  const response = fetch(`${BASE_URL}/test`);
...
console.log("\n🔍 Test 2: GET with Query Parameters");
try {
  const response = fetch(`${BASE_URL}/test/query?param1=value1&param2=value2&test=true`);
...
try {
  ...
  const response = fetch(`${BASE_URL}/test/json`, {
    method: "POST",
    headers: {
...
console.log("\n🔍 Test 7: Custom Headers Validation");
try {
  const response = fetch(`${BASE_URL}/test/headers`, {
    headers: {
...
for (const statusCode of statusCodes) {
  try {
    const response = fetch(`${BASE_URL}/test/status/${statusCode}`);
    const data = safeParseJSON(response.text());
...
try {
  ...
  const response = fetch(`${BASE_URL}/test/validate`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
